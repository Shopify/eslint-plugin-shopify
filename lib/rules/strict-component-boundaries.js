...
function isSupportedFileType(pathParts) {
  const unSupportedType = ['.json', '.graphql'];
  const lastPathPart = pathParts[pathParts.length - 1];
  return !unSupportedType.some((type) => lastPathPart.includes(type));
}
